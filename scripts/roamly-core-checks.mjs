...
[
  ...
  "roamly_finalize_generation_trip",
  "p_unlock_source",
  "p_completed_at",
  "completeDayStates",
...
assert.ok(liveTripClient.includes('"Enable notifications"'), "field test must expose an Enable notifications control");
assert.ok(liveTripClient.includes("requestNotificationPermission()"), "field test notification permission must be requested by the control handler");
assert.ok(liveTripClient.includes('notificationPermission === "denied"'), "field test must explain denied notification permission");
assert.ok(liveTripClient.includes('notificationPermission === "granted" && !pushReady'), "field test must distinguish granted permission from server registration");
assert.ok(liveTripClient.includes('setNotice("Notifications ready")'), "field test ready state must follow successful registration");
assert.ok(liveTripClient.includes("setPushReady(true);\n      setNotice(\"Notifications ready\")"), "field test must require server registration before showing ready");
assert.ok(liveTripClient.includes("currentPushReady &&") && liveTripClient.includes("currentNotificationPermission === \"granted\""), "valid field-test setup must be restored without repeated registration");
assert.ok(pushClient.includes("getSubscription()") && pushClient.includes("pushManager.subscribe"), "field test must use a real PushManager subscription");
...
assert.ok(adminLiveTestRoute.includes("requireRoamlyAdmin"), "Push diagnostic must require existing Admin authentication");
assert.ok(adminLiveTestRoute.includes("send_test_push_notification"), "Push diagnostic action must be wired through the Admin route");
assert.ok(adminLiveTestConsole.includes("Phone push") && adminLiveTestConsole.includes("Send test notification"), "Admin must show the phone push status and send control");
assert.ok(adminLiveTestConsole.includes('subscription.trip_id === selectedTrip?.id'), "Admin push status must be scoped to the selected prepared trip");
assert.ok(liveCompanionTest.includes('eq("trip_id", tripId)') && liveCompanionTest.includes('title: "Roamly Test"') && liveCompanionTest.includes('body: "Push notifications are working on this phone."'), "Push diagnostic must target the prepared phone and use the required copy");
assert.ok(liveCompanionTest.includes("createNotification: false") && liveCompanionTest.includes("sendEmail: false") && liveCompanionTest.includes("subscriptionIds"), "Push diagnostic must not create an in-app or email notification and must target one subscription");
assert.ok(pushServer.includes("subscriptionIds") && pushServer.includes("webpush.sendNotification"), "Push diagnostic must reuse the production server push implementation");
assert.ok(serviceWorker.includes("self.addEventListener(\"push\"") && serviceWorker.includes("showNotification"), "Push diagnostic must use the production service worker push handler");
assert.ok(!adminLiveTestConsole.includes("subscription.endpoint") && !adminLiveTestConsole.includes("subscription.p256dh") && !adminLiveTestConsole.includes("subscription.auth"), "Admin push diagnostic must not expose subscription secrets");
const pushScopeMigration = read("supabase/migrations/20260907_roamly_field_test_push_scope.sql");
assert.ok(pushScopeMigration.includes("add column if not exists trip_id"), "Push subscription scope migration must be present");
