...
export function enforceSelectedHotelIdentity(itinerary: RoamlyItinerary, state: SelectedHotelState): RoamlyItinerary {
  if (hasConfirmedHotelBooking(state)) return itinerary;
  const candidate = selectedHotelCandidate(state);
  const candidateId = text(candidate?.candidateId);
  const name = text(candidate?.name);
  if (!candidate || !candidateId || !name) return itinerary;
  ...
  const daily_itinerary = itinerary.daily_itinerary.map((day) => ({
    ...day,
  ...
  const booking_suggestions = [...itinerary.booking_suggestions];
