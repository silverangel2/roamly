...
export function selectCampaignPhotoAsset<T extends CampaignPhotoCandidate>(
  assets: T[],
  destination: string,
  topic: string
) {
  ...
  return [...assets]
    .filter((asset) => {
      ...
      // Explicit provenance is authoritative. A destination mismatch can never
      // be rescued by a broad topic/title/source match.
      ...
      return Boolean(
        ...
        (assetTopic && assetTopic === topicKey)
      );
    })
    .sort((a, b) => {
