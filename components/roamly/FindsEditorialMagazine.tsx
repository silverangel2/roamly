...
export function FindsEditorialMagazine({ cards, destination, emptyMessage, disclosures, liveTools, searchOpen, onSearchOpenChange, onOpenSearch }: { cards: FindsCard[]; destination: string; emptyMessage: string; disclosures: string[]; liveTools: ReactNode; searchOpen: boolean; onSearchOpenChange: (open: boolean) => void; onOpenSearch: (tab: "stays" | "flights" | "activities") => void }) {
  ...
  return <section className="mt-10" aria-label="Editorial travel magazine">
    ...
    <section className="mt-12" aria-labelledby="stay-heading"><div className="mb-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#0f6e66]">A place to begin</p><h2 id="stay-heading" className="mt-2 text-2xl font-black tracking-tight text-[#203c43]">Where to stay</h2></div>{hotels[0] ? <><LiveCard card={hotels[0]} kind="hotel" featured /><div className="mt-4"><a href={findsTravelServices.stays.href} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center rounded-full border border-[#b9d7c5] bg-white px-5 text-sm font-black text-[#0f6e66] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#0f6e66]/20">Explore more stays<span aria-hidden="true" className="ml-2">↗</span></a></div></> : <><EmptyOpportunity message="Real properties, property photos, and date-specific prices appear here when a verified stay search returns them. We don’t invent hotels or availability." action="Check stays" onAction={() => onOpenSearch("stays")} /><div className="mt-4"><a href={findsTravelServices.stays.href} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center rounded-full border border-[#b9d7c5] bg-white px-5 text-sm font-black text-[#0f6e66] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#0f6e66]/20">Explore stays<span aria-hidden="true" className="ml-2">↗</span></a></div></>}</section>

    <section className="mt-12" aria-labelledby="flights-heading"><div className="mb-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#0f6e66]">Travel ideas, with real fares</p><h2 id="flights-heading" className="mt-2 text-2xl font-black tracking-tight text-[#203c43]">Flights worth checking</h2></div>{flights.length ? <div className="space-y-4">{flights.slice(0, 3).map((card) => <EditorialFlight key={card.id} card={card} />)}</div> : <EmptyOpportunity message="Route and fare references appear when the approved flight feed returns them. Prices and schedules are never guessed." action="Check flights" onAction={() => onOpenSearch("flights")} />}<div className="mt-6"><FindsWidgetSection config={findsWidgets.flights} /></div></section>

    <section className="mt-12" aria-labelledby="activities-heading"><div className="mb-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#0f6e66]">Make a day of it</p><h2 id="activities-heading" className="mt-2 text-2xl font-black tracking-tight text-[#203c43]">Things worth doing</h2></div>{activities.length ? <><LiveCard card={activities[0]} kind="activity" featured />{activities.length > 1 ? <div className="mt-4 flex snap-x gap-4 overflow-x-auto pb-3">{activities.slice(1, 6).map((card) => <LiveCard key={card.id} card={card} kind="activity" />)}</div> : null}</> : <EmptyOpportunity message="Grounded activities and public events appear here when their source returns real details. Activity photos are shown only when supplied by the source." action="Explore experiences" onAction={() => onOpenSearch("activities")} />}<div className="mt-8 grid gap-6 lg:grid-cols-2"><FindsWidgetSection config={findsWidgets.experiences107} /><FindsWidgetSection config={findsWidgets.experiences121} /></div></section>

    <section className="mt-12"><PhotoStory story={secondaryStory} /></section>

    <section className="mt-12" aria-labelledby="travel-tools-heading"><div className="mb-6"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#0f6e66]">Useful extras for the journey</p><h2 id="travel-tools-heading" className="mt-2 text-2xl font-black tracking-tight text-[#203c43]">More ways to explore</h2></div><div className="grid gap-6 lg:grid-cols-2"><FindsPromo promo={activeFindsPromo} /><FindsTrackedLink href={findsTravelServices.airportTransfer.href} eyebrow={findsTravelServices.airportTransfer.eyebrow} headline={findsTravelServices.airportTransfer.headline} description={findsTravelServices.airportTransfer.description} action="Find an airport transfer" /></div><div className="mt-8 grid gap-8 lg:grid-cols-2"><FindsWidgetSection config={findsWidgets.esim} /><FindsWidgetSection config={findsWidgets.travel} /><FindsWidgetSection config={findsWidgets.cars} /></div></section>
