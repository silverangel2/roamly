...
export function ActiveTripPanel({ trip }: { trip: TrackingTrip | null }) {
  ...
  return (
    <div className="rounded-[1.5rem] border border-cloud bg-white/90 p-5 shadow-soft">
      ...
      <h2 className="mt-2 text-2xl font-black text-ink">{trip.title || trip.destination || trip.destination_name}</h2>
      <p className="mt-2 text-sm font-bold text-slate-500">
        {trip.destination_city || trip.destination || "Destination ready"} · {trip.status}
      </p>
    </div>
  );
}
