  ...
  return (
    <section className="mt-10" aria-label="Travel finds">
      <div className="flex flex-col gap-4 border-b border-[#e2e9e2] pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-xs font-black uppercase tracking-[0.16em] text-[#0f6e66]">Explore the trip</p><h2 className="mt-1 text-2xl font-black tracking-tight">A good trip starts with a good find.</h2><p className="mt-1 text-sm text-[#526b62]">Real options for {destination === "your next somewhere" ? "your next trip" : destination}.</p></div>
        <div className="max-w-sm text-xs leading-5 text-[#526b62] sm:text-right"><p className="font-semibold">Prices, photos, and availability come from the provider.</p>{destination !== "your next somewhere" || origin || startDate || endDate ? <p className="mt-1 text-[#7a8c84]">{[origin ? `From ${origin}` : "", startDate ? `${dateLabel(startDate)}${endDate ? `–${dateLabel(endDate)}` : ""}` : ""].filter(Boolean).join(" · ") || "Trip context saved"}</p> : <p className="mt-1 text-[#7a8c84]">Add trip details to make these shelves more relevant.</p>}</div>
      </div>
      ...
      {visible.length === 0 ? <div key={`empty-${active}`} id="finds-panel" role="tabpanel" aria-labelledby={`finds-tab-${active}`} className="rounded-[1.5rem] border border-[#e1e9e2] bg-white px-6 py-12 text-center shadow-[0_12px_35px_rgba(39,68,58,0.04)]"><span aria-hidden="true" className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#eff7ef] text-2xl text-[#0f6e66]">✦</span><h3 className="mt-4 text-xl font-black tracking-tight text-[#203c43]">No verified listings in this shelf yet</h3><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#657971]">{active === "amazon" ? emptyMessage : active === "all" ? "No verified travel finds are available yet. Choose a shelf and search for current stays, flights, experiences, or gear. Offers appear only when their details, photos, and provider links can be verified." : active === "stays" ? "Use the stay search above. Only real, priced stays with a verified property photo and a direct provider booking link appear here." : "We’ll only open this shelf when a partner can provide a real listing, its own photo, current details, and a direct place to book. No placeholders, scraped stock photos, or pretend offers."}</p></div> : <div key={`listings-${active}`} id="finds-panel" role="tabpanel" aria-labelledby={`finds-tab-${active}`} className="grid gap-5 pt-1 sm:grid-cols-2 xl:grid-cols-3">
        {visible.map((card, index) => <article key={card.id} className="group overflow-hidden rounded-[1.35rem] border border-[#e3e9e3] bg-white shadow-[0_5px_22px_rgba(39,68,58,0.045)] transition duration-300 hover:-translate-y-1 hover:border-[#bfd8ca] hover:shadow-[0_18px_42px_rgba(39,88,80,0.12)] motion-reduce:transform-none motion-reduce:transition-none">
          <div className={`relative h-64 overflow-hidden sm:h-72 ${card.category === "hotel" || card.category === "activity" ? "bg-[#e9f1eb]" : "bg-[#f8faf7] p-4"}`}>
            <Image src={card.image} alt={card.imageAlt} fill unoptimized loading={index === 0 ? "eager" : "lazy"} sizes="(min-width: 1280px) 30vw, (min-width: 640px) 45vw, 100vw" className={`${card.category === "hotel" || card.category === "activity" ? "object-cover" : "object-contain p-7"} transition duration-500 group-hover:scale-[1.04] motion-reduce:transform-none`} />
          ...
          <div className="flex min-h-[216px] flex-col p-5">
            ...
            <p className="mt-2 line-clamp-3 text-xs leading-5 text-[#75857d]">{card.description}</p>
            ...
            <div className="mt-auto flex items-center justify-between gap-3 pt-5"><span className="text-xs font-bold text-[#83938c]">{providerLabel(card.provider)}</span><a href={card.href} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center rounded-full bg-[#0f6e66] px-5 text-xs font-black text-white transition hover:bg-[#0e605a] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#0f6e66]/20">{card.action}<span aria-hidden="true" className="ml-2">↗</span></a></div>
          </div>
        </article>)}
      </div>}
