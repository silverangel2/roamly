      ...
      <div className="mx-auto max-w-7xl">
        <section className="relative isolate overflow-hidden rounded-[2rem] bg-[#eaf4ed] px-6 py-9 sm:px-10 sm:py-12 lg:px-14">
          ...
          <div className="max-w-3xl">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-[#16877f]">The Roamly travel market</p>
            <h1 className="mt-3 max-w-2xl text-4xl font-black leading-[1.02] tracking-[-0.055em] sm:text-6xl">Find the things that make the trip <span className="text-[#16877f]">unforgettable.</span></h1>
          ...
          <form action="/finds" className="mt-7 flex flex-col gap-2 rounded-2xl border border-white/80 bg-white p-2 shadow-[0_18px_55px_rgba(39,88,80,0.12)] sm:max-w-3xl sm:flex-row">
            ...
            <input id="finds-query" name="q" defaultValue={search.q || ""} placeholder="Search bags, adapters, travel essentials…" maxLength={100} className="min-h-12 min-w-0 flex-1 rounded-xl px-3 text-sm text-[#203c43] outline-none placeholder:text-[#92a098] focus:ring-4 focus:ring-[#16877f]/10" />
            <button className="min-h-12 rounded-xl bg-[#16877f] px-6 text-sm font-extrabold text-white transition hover:bg-[#11756e] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#16877f]/25" type="submit">Find your gear <span aria-hidden="true" className="ml-1">→</span></button>
          </form>
          ...
          <nav aria-label="Shop by travel category" className="mt-5 flex flex-wrap gap-2">
            {[
              ...
              ["amazon", "Travel gear", "◇"]
            ].map(([id, label, icon]) => <a key={id} href={`#finds-tab-${id}`} className="inline-flex min-h-10 items-center gap-2 rounded-full border border-white/90 bg-white/75 px-4 text-xs font-extrabold text-[#31594f] shadow-sm transition hover:-translate-y-0.5 hover:bg-white hover:shadow-md motion-reduce:transform-none"><span aria-hidden="true" className="text-sm text-[#16877f]">{icon}</span>{label}<span aria-hidden="true" className="text-[#8ba197]">→</span></a>)}
          </nav>

          <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-xs font-bold text-[#557369]">
            <span><span className="mr-1.5 text-[#16877f]">✓</span>Real listings and item photos</span>
            <span><span className="mr-1.5 text-[#16877f]">✓</span>Current details from the source</span>
            <span><span className="mr-1.5 text-[#16877f]">✓</span>Checkout stays with the provider</span>
          </div>
        </section>
        ...
        <section className="mt-8 flex flex-col justify-between gap-4 rounded-[1.6rem] bg-[#e8f4ec] p-6 sm:flex-row sm:items-center sm:p-8">
          <div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[#16877f]">A little travel daydream</p><h2 className="mt-2 text-2xl font-black tracking-tight">Can you guess the city from three clues?</h2><p className="mt-2 text-sm text-[#5c716c]">A tiny daily puzzle. No account or booking required.</p></div>
