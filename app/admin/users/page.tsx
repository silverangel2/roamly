...
export default async function AdminUsersPage() {
  ...
  const [{ data }, { data: entitlements }, { data: purchases }] = await Promise.all([
    state.admin
          ...
          <Card key={user.id} className="p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-xs font-black uppercase tracking-[0.12em] text-slate-500">
              <span className="rounded-full bg-mist px-3 py-2">
                {entitlementByUser.get(user.id)?.free_itinerary_used_at ? "free used" : "free available"}
              </span>
              <span className="rounded-full bg-mist px-3 py-2">
                paid itinerary {purchasesByUser.get(user.id)?.filter((item) => item.status === "paid" && ["itinerary", "itinerary_unlock"].includes(item.purchase_type)).length || 0}
              </span>
              <span className="rounded-full bg-mist px-3 py-2">
                companion {purchasesByUser.get(user.id)?.filter((item) => item.status === "paid" && ["features", "tracking_addon"].includes(item.purchase_type)).length || 0}
              </span>
              <span className="rounded-full bg-mist px-3 py-2">
                complete {purchasesByUser.get(user.id)?.filter((item) => item.status === "paid" && ["complete_trip", "bundle"].includes(item.purchase_type)).length || 0}
              </span>
            </div>
          </Card>
        ))}
        {!data?.length ? <Card>No user profiles yet.</Card> : null}
      </div>
    </main>
  );
}
