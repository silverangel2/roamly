...
export async function POST(request: Request, context: RouteContext) {
  ...
  const result = await auth.supabase.rpc("roamly_reserve_customer_trip_destination_change", { p_proposal_id: proposalId, p_trip_id: tripId });
  if (result.error) {
    const stale = result.error.code === "40001" || result.error.message.includes("DESTINATION_CHANGE_");
    return NextResponse.json({ ok: false, error: stale ? "The trip changed while this request was waiting. Review the destination again." : "The destination change could not be started." }, { status: stale ? 409 : 400 });
  }
  ...
  if (successorTripId && result.data?.status === "generating" && result.data?.idempotent !== true) {
    try {
      ...
      if (admin) await admin.rpc("roamly_fail_customer_trip_destination_change", { p_proposal_id: proposalId, p_user_id: auth.user.id, p_successor_trip_id: successorTripId, p_reason: error instanceof Error ? error.message : "GENERATION_START_FAILED" });
    ...
    scheduleStagedGenerationAdvance({ tripId: successorTripId, origin: new URL(request.url).origin, reason: "customer_destination_change_successor", requestId: proposalId });
  }
